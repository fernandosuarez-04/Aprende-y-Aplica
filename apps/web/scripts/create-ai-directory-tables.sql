-- Script para crear las tablas del Directorio de IA
-- Incluye tablas para prompts y apps de IA
...
CREATE TRIGGER update_ai_categories_updated_at BEFORE UPDATE ON ai_categories FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TRIGGER update_ai_apps_updated_at BEFORE UPDATE ON ai_apps FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TRIGGER update_app_ratings_updated_at BEFORE UPDATE ON app_ratings FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
BEGIN
    UPDATE ai_prompts 
    SET 
        rating = (SELECT AVG(rating) FROM prompt_ratings WHERE prompt_id = NEW.prompt_id),
...
BEGIN
    UPDATE ai_apps 
    SET 
        rating = (SELECT AVG(rating) FROM app_ratings WHERE app_id = NEW.app_id),
        rating_count = (SELECT COUNT(*) FROM app_ratings WHERE app_id = NEW.app_id)
...
CREATE TRIGGER update_app_rating_trigger 
    AFTER INSERT OR UPDATE OR DELETE ON app_ratings 
