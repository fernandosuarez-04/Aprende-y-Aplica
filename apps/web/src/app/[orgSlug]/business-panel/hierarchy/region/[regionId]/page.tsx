...
export default function RegionDetailPage() {
  ...
  // Handlers
  const handleUpdateRegion = async (data: Partial<Region>) => {
    if (!region) return
    try {
      setIsLoading(true)
      const result = await HierarchyService.updateRegion(region.id, data)
  ...
  const handleDeleteRegion = async () => {
    ...
    try {
      setIsLoading(true)
      const result = await HierarchyService.deleteRegion(region.id)
      if (result.success) {
  ...
  const handleCreateZone = async (data: Partial<Zone> & { region_id: string }) => {
    try {
      setIsLoading(true)
      const result = await HierarchyService.createZone(data as any)
      if (result.success && result.data) {
  ...
  const handleLogoUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setIsUploading(true)
    try {
      const result = await uploadRegionLogo(regionId, file)
      
      if (result.success && result.url) {
        await HierarchyService.updateRegion(regionId, { logo_url: result.url } as any)
  ...
  const mapPoints = zones
    ...
    .map(z => ({
      ...
      isTopPerformer: analytics?.top_performer?.id === z.id,
      stats: {
        value: z.users_count || 0,
        label: 'Usuarios'
      }
    }))
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-20">
        ...
        <div className="relative p-8">
          <div className="flex flex-col md:flex-row gap-8 items-start">
            ...
            <div className="relative shrink-0">
              ...
              >
                {(region as any).logo_url ? (
                  <Image
                    src={(region as any).logo_url}
                    alt={region.name}
                    fill
            ...
            >
              ...
              {region.manager ? (
                <div className="text-center">
                  ...
                  <h4 className="text-xl font-bold text-white mb-1">{region.manager.display_name}</h4>
                  <p className="text-white/50 text-sm mb-6">{region.manager.email}</p>
                  
                  <div className="grid grid-cols-2 gap-3 text-left">
                    <div className="p-3 rounded-xl bg-white/5 border border-white/5">
                      <p className="text-xs text-white/40 mb-1">Teléfono</p>
                      <p className="text-sm font-medium text-white break-all">
                        {region.phone || 'No registrado'}
                      </p>
                    </div>
                    <div className="p-3 rounded-xl bg-white/5 border border-white/5">
                      <p className="text-xs text-white/40 mb-1">Email Contacto</p>
                      <p className="text-sm font-medium text-white break-all">
                        {region.email || 'No registrado'}
                      </p>
                    </div>
                  </div>
                </div>
      ...
      {region && (
        <ZoneForm
          isOpen={isNewZoneOpen}
          onClose={() => setIsNewZoneOpen(false)}
      ...
      <DeleteConfirmModal
        ...
        title="Eliminar Región"
        message="¿Estás seguro de que deseas eliminar esta región? Esta acción eliminará también todas las zonas y equipos asociados."
