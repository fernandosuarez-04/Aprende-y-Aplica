...
export function ContextualVoiceGuide({
  ...
  const voice = useVoiceAgent({
    ...
    Estás guiando al usuario en un tour por la plataforma.
    Responde de forma breve, clara y amigable a las preguntas del usuario.`,
    onError: (error) => {
      console.error('🔴 [ContextualVoiceGuide] Error en voice agent:', error);
    },
  });
  ...
  useEffect(() => {
    console.log('🎙️ [ContextualVoiceGuide] Estado de voz:', {
  ...
  useEffect(() => {
    console.log('🔌 [ContextualVoiceGuide] Conectando a Gemini...');
    voice.connect().then(() => {
      console.log('✅ [ContextualVoiceGuide] Conectado a Gemini exitosamente');
    }).catch((error) => {
      console.error('❌ [ContextualVoiceGuide] Error conectando a Gemini:', error);
    });

    return () => {
      console.log('🔌 [ContextualVoiceGuide] Desconectando de Gemini...');
      voice.disconnect();
    };
  }, []);
  ...
  // Listener para abrir el tour manualmente
  useEffect(() => {
  ...
  useEffect(() => {
    if (isVisible && currentStep === 0 && isAudioEnabled && voice.isConnected) {
      const timer = setTimeout(() => {
        console.log('🔊 [ContextualVoiceGuide] Reproduciendo paso inicial:', ONBOARDING_STEPS[0].speech);
        voice.speak(ONBOARDING_STEPS[0].speech).catch((err) => {
          console.error('❌ [ContextualVoiceGuide] Error reproduciendo paso inicial:', err);
        });
  ...
  const toggleListening = async () => {
    console.log('🎤 [ContextualVoiceGuide] Toggle listening. Estado actual:', voice.isListening);

    if (voice.isListening) {
      console.log('⏹️ [ContextualVoiceGuide] Deteniendo escucha...');
      voice.stopListening();
    } else {
      try {
        console.log('🎙️ [ContextualVoiceGuide] Iniciando escucha...');
        console.log('🔍 [ContextualVoiceGuide] Estado de conexión:', voice.connectionState);
        console.log('🔍 [ContextualVoiceGuide] ¿Está conectado?:', voice.isConnected);

        if (!voice.isConnected) {
          console.warn('⚠️ [ContextualVoiceGuide] No conectado. Intentando conectar...');
          await voice.connect();
          console.log('✅ [ContextualVoiceGuide] Conectado exitosamente');
        }

        voice.setIsProcessing(false);
        await voice.startListening();
        console.log('✅ [ContextualVoiceGuide] Escucha iniciada correctamente');
        setTranscript('');
      } catch (error: any) {
        console.error('❌ [ContextualVoiceGuide] Error al iniciar escucha:', error);
        console.error('❌ [ContextualVoiceGuide] Error detallado:', {
          name: error?.name,
  ...
  const handleVoiceQuestion = async (question: string) => {
    ...
    if (processingRef.current) {
      console.warn('⚠️ [ContextualVoiceGuide] Ya hay una pregunta procesándose, ignorando.');
      return;
    }

    console.log('💬 [ContextualVoiceGuide] Procesando pregunta:', question);
    ...
    if (lastUserMsg) {
      ...
      if (recent && (lastText === question || lastText.includes(question) || question.includes(lastText))) {
        console.warn('⚠️ [ContextualVoiceGuide] Pregunta similar ya procesada, ignorando.');
    ...
    try {
      console.log('🤖 [ContextualVoiceGuide] Enviando pregunta a LIA...');
      ...
      console.log('💬 [ContextualVoiceGuide] Respuesta de LIA:', liaResponse);
      ...
      console.log('🔊 [ContextualVoiceGuide] Reproduciendo respuesta de LIA...');
      await voice.speak(liaResponse);
      console.log('✅ [ContextualVoiceGuide] Respuesta reproducida');

    } catch (error) {
      console.error('❌ [ContextualVoiceGuide] Error procesando pregunta:', error);
      const errorMessage = t('onboarding.voice.errorProcessing');
      try {
        await voice.speak(errorMessage);
      } catch(e) {
        console.error('❌ [ContextualVoiceGuide] Error reproduciendo mensaje de error:', e);
      }
    } finally {
      processingRef.current = false;
      voice.setIsProcessing(false);
  ...
  const handleNext = () => {
    ...
    if (nextStep < ONBOARDING_STEPS.length) {
      ...
      console.log('➡️ [ContextualVoiceGuide] Avanzando al paso:', nextStep);
      voice.speak(ONBOARDING_STEPS[nextStep].speech).catch((err) => {
        console.error('❌ [ContextualVoiceGuide] Error reproduciendo paso:', err);
      });
    } else {
      handleComplete();
    }
  ...
  const handlePrevious = () => {
    ...
    if (currentStep > 0) {
      ...
      console.log('⬅️ [ContextualVoiceGuide] Retrocediendo al paso:', prevStep);
