...
export interface AppRatingInlineProps {
  /**
   * App slug
   */
  appSlug: string;
   ...
   * @param newRatingCount - New rating count
   */
  onRatingSubmitted?: (newRating: number, newRatingCount: number) => void;
}

export function AppRatingInline({
  appSlug,
  currentRating,
  currentRatingCount,
  onRatingSubmitted,
}: AppRatingInlineProps) {
  ...
  useEffect(() => {
    async function checkRating() {
      try {
        setIsChecking(true);
        const ratingCheck = await AppRatingService.checkUserRating(appSlug);
  ...
  const handleRatingChange = async (rating: number) => {
    ...
    try {
      const result = await AppRatingService.submitRating(appSlug, rating);
      ...
      <div className="flex items-center gap-4">
        <div className="flex items-center gap-2">
          <span className="text-sm font-medium text-gray-700 dark:text-gray-300">
            {userRating !== null ? 'Tu calificación:' : 'Califica esta app:'}
          </span>
        </div>
        <StarRating
          rating={userRating || 0}
