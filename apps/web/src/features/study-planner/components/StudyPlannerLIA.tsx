...
import { parseSofLIAResponseToSchedules } from '../services/plan-parser.service';
...
export function StudyPlannerSofLIA() {
 ...
 console.error('[ERROR] [Insert Events] Error:', error);
 setInsertResult({
 success: false,
 ...
 const [SofLIAConversationId, setSofLIAConversationId] = useState<string | null>(null);

 // ✅ NUEVO: Hook para datos de SofLIA (lecciones pendientes desde BD)
 const SofLIAData = useSofLIAData();
 ...
 const getStorageKey = (userId: string) => `SofLIA_planner_session_v1_${userId}`;
 ...
 const title = trimmed.replace(/^[ðŸŽ¯📈📚💡🗓â°📋✅[ERROR] [WARN] ]*\s*/, '').replace(/\*\*/g, '').replace(/:/g, '').trim();
 ...
 flushList();
 flushParagraph();
 const subtitle = trimmed.replace(/^[ðŸŽ¯📈📚💡🗓â°📋✅[ERROR] [WARN] ]*\s*/, '').replace(/\*\*/g, '').replace(/:/g, '').trim();
 ...
 console.error('[ERROR] [OAuth Return] Error reanudando flujo:', error);
 }
 };

 resumeFlow();
 } else if (calendarError) {
 console.error('[ERROR] [OAuth Return] Error en conexión:', calendarError);
 // Limpiar URL
 window.history.replaceState({}, '', window.location.pathname);
 setConversationHistory(prev => [...prev, {
 role: 'assistant',
 ...
 if (assignedCourses.length > 0 && !SofLIAData.isReady && !SofLIAData.isLoading && !SofLIAData.error) {
 SofLIAData.loadPendingLessons();
 }
 }, [assignedCourses, SofLIAData.isReady, SofLIAData.isLoading, SofLIAData.error, SofLIAData.loadPendingLessons]);

 // Sincronizar datos del hook con las refs/estados existentes del componente
 useEffect(() => {
 if (SofLIAData.isReady && SofLIAData.lessons.length > 0) {
 // Mapear al formato esperado por el componente
 const formattedLessons = SofLIAData.lessons.map(lesson => ({
 courseId: lesson.courseId,
 courseTitle: lesson.courseTitle,
 lessonId: lesson.lessonId,
 lessonTitle: lesson.lessonTitle, // [WARN]  NOMBRE EXACTO DE LA BD - NO ALUCINAR
 ...
 }, [SofLIAData.isReady, SofLIAData.lessons]);
 ...
 if (assignedCourses.length > 0 && !SofLIAData.isReady) {
 return;
 }
...
8. Usa markdown para dar formato (negritas, listas con viñetas, etc.)`;
 ...
 welcomeDueDateContext = `\n\n🚨 FECHA LÃMITE OBLIGATORIA: ${dueDateFormattedWelcome}\n[WARN]  Todas las lecciones DEBEN completarse ANTES de esta fecha.`;
 }

 // Obtener contexto de lecciones pendientes
 const lessonsContext = SofLIAData.getLessonsForPrompt();
 ...
 const SofLIASystemPrompt = generateStudyPlannerPrompt({
 ...
 conversationHistory: [],
 systemPrompt: SofLIASystemPrompt,
 ...
 if (response.ok) {
 const data = await response.json();
 let SofLIAResponse = data.response;

 // ✅ Guardar conversationId para analytics
 if (data.conversationId) {
 setSofLIAConversationId(data.conversationId);
 }

 // Agregar el mensaje de SofLIA al historial
 setConversationHistory([{ role: 'assistant', content: SofLIAResponse }]);
 ...
 }, [showConversation, conversationHistory.length, showCourseSelector, userContext, assignedCourses, connectedCalendar, SofLIAData.isReady, isRunning]);
 ...
 // âš™ CONFIGURACIÓN DE VOZ ELEVENLABS - Optimizada para máxima expresión y consistencia
 ...
 console.error('[ERROR] [speakText] Error al reproducir audio:', playError);
 setIsSpeaking(false);
 }
 } catch (error: any) {
 ...
 // [WARN]  MUY CONSERVADOR: Solo filtrar si COMIENZA con cabeceras del prompt

 // Solo filtrar si COMIENZA con cabeceras ASCII del prompt
 const startsWithPrompt =
 SofLIAResponse.trim().startsWith('â•”â•â•â•') ||
 SofLIAResponse.trim().startsWith('â–ˆ IDENTIDAD') ||
 SofLIAResponse.trim().startsWith('â–ˆ DATOS') ||
 SofLIAResponse.trim().startsWith('PROMPT MAESTRO') ||
 SofLIAResponse.trim().startsWith('â›” INSTRUCCIÓN CRÃTICA');

 if (startsWithPrompt) {
 console.warn(' [handleQuestion] Respuesta COMIENZA con prompt del sistema');
 SofLIAResponse = '¡Perfecto! Vamos a continuar. ¿Qué más necesitas para tu plan de estudios?';
 }
 ...
 const shouldAddAssistant = !(last && last.role === 'assistant' && last.content === SofLIAResponse);

 let next = prev.slice();
 if (shouldAddUser) next = [...next, { role: 'user', content: question }];
 if (shouldAddAssistant) next = [...next, { role: 'assistant', content: SofLIAResponse }];
 return next;
 });

 // Detectar si SofLIA está pidiendo seleccionar cursos y abrir el modal automáticamente
 if (SofLIAResponse.includes('¿Qué cursos te gustaría incluir?') ||
 SofLIAResponse.includes('qué cursos') ||
 SofLIAResponse.includes('seleccionar cursos')) {
 ...
 await speakText(SofLIAResponse);
 ...
 const SofLIAResponse = `¡Excelente elección! Has seleccionado ${selectedCourses.length} curso${selectedCourses.length > 1 ? 's' : ''}: ${courseNames}.\n\nAntes de crear tu plan de estudios personalizado, necesito conocer tu preferencia de ritmo de estudio.`;

 setConversationHistory(prev => [...prev, { role: 'assistant', content: SofLIAResponse }]);
 ...
 const SofLIAResponse = 'Parece que no seleccionaste ningún curso. ¿Te gustaría ver tus cursos disponibles de nuevo o prefieres decirme qué temas te interesan?';
 setConversationHistory(prev => [...prev, { role: 'assistant', content: SofLIAResponse }]);
 ...
 authUrl,
 'SOFLIASofLIA-ai-google-calendar-auth',
 ...
 console.warn('[WARN] [Calendar] Calendario no encontrado en BD, reintentando en 1 segundo...');
 ...
 console.error('[ERROR] [Calendar] Error verificando estado del calendario:', error);
 setIsConnectingCalendar(false);
 }
 };
 ...
 console.error('[ERROR] [Calendar] Error al conectar calendario:', event.data.error);
 ...
 console.warn('[WARN] [handleApproachSelection] Error HTTP verificando calendario:', calendarResponse.status);
 }
 } catch (error) {
 console.error('[ERROR] [handleApproachSelection] Error verificando calendario:', error);
 }
 } else {
 }
...
[WARN]  INSTRUCCIONES IMPORTANTES - NO pedir calendario:
1. Confirma la selección del tipo de sesiones de manera entusiasta
...
6. Usa markdown y emojis con moderación`;
 ...
 console.error('[ERROR] [handleApproachSelection] Error en analyzeCalendarAndSuggest:', error);
 ...
 console.error('[ERROR] [B2B] Error obteniendo eventos:', error);
 }
 ...
 console.warn(`[WARN] [B2B] No se pudo obtener progreso del curso ${course.courseId}`);
 }
 } catch (progressError) {
 console.warn(`[WARN] [B2B] Error obteniendo progreso del curso ${course.courseId}:`, progressError);
 }
 ...
 console.warn(`[WARN] [B2B] No se encontraron módulos en metadata para curso ${course.courseId}`);
 }
 } else {
 console.warn(`[WARN] [B2B] No se pudo obtener metadata del curso ${course.courseId}`);
 }
 } catch (error) {
 console.warn(`[WARN] [B2B] Error obteniendo lecciones del curso ${course.courseId}:`, error);
 }
 ...
 console.error('[ERROR] [B2B] Error interno en analyzeCalendarAndSuggest:', innerError);
 ...
 console.error('[ERROR] [B2B] Error en análisis de calendario:', error);
 setIsProcessing(false);
 ...
 if (isProcessing) {
 console.warn('[WARN] [analyzeCalendarAndSuggest] Se llamó mientras estaba procesando. Continuando de todos modos para asegurar recuperación...');
 ...
 if (!targetDateObjForEvents) {
 console.warn(`[WARN] No se pudo parsear la fecha objetivo: "${effectiveTargetDate}"`);
 }
 } catch (e) {
 console.warn('[ERROR] Error parseando fecha objetivo para eventos:', e);
 }
 }
 ...
 console.error('[ERROR] Error en respuesta de eventos:', eventsResponse.status, errorData);
 ...
 console.warn('[WARN] Token expirado y no se pudo refrescar. Se requiere reconexión del calendario.');
 setConnectedCalendar(null);
 ...
 calendarEvents = [];
 console.warn('[WARN] No se pudieron obtener eventos del calendario, continuando sin análisis de disponibilidad');
 }
 }
 } catch (calError) {
 ...
 console.warn(` [WARN] targetDateObjForEvents es NULL - usando 30 días por defecto`);
 }
 ...
 */
 ...
 console.warn(`[WARN] weeksUntilTarget es menor a 1, ajustando a 1`);
 weeksUntilTarget = 1;
 } else if (weeksUntilTarget > 52) {
 console.warn(`[WARN] weeksUntilTarget es mayor a 52 semanas, ajustando a 52`);
 weeksUntilTarget = 52;
 }

 } else {
 console.warn(`[WARN] No se pudo parsear la fecha objetivo: ${targetDate}`);
 ...
 console.warn(` [WARN] Lección duplicada detectada en API (cálculo): ${lesson.lessonId} - ${lesson.lessonTitle}`);
 }
 }
 });
 const publishedLessons = Array.from(uniqueLessonsMap.values());
 ...
 console.warn(` [WARN] Error obteniendo progreso del curso ${courseId}: ${errorData.error || progressResponse.status}`);
 }
 } catch (progressError) {
 ...
 console.warn(`[WARN] totalLessonsNeeded es 0, usando estimación conservadora de 10 lecciones por curso`);
 ...
 console.warn('[WARN] No se pudo calcular lecciones necesarias: selectedCourseIds.length =', selectedCourseIds.length, 'profileAvailability =', !!profileAvailability);
 // Fallback: estimación conservadora
 if (selectedCourseIds.length > 0) {
 totalLessonsNeeded = selectedCourseIds.length * 10;
 }
 ...
 if (!weeklyGoals) {
 console.warn('[WARN] No se pudieron calcular las metas semanales. Verificar condiciones.');
 ...
 console.warn(` [WARN] Lección inválida en módulo ${module.moduleId}:`, lesson);
 return null;
 }
 ...
 console.warn(` [WARN] Lección duplicada detectada en API: ${lesson.lessonId} - ${lesson.lessonTitle}`);
 }
 }
 });
 ...
 if (!isValid) {
 console.warn(` [WARN] Lección filtrada por datos inválidos:`, lesson);
 }
 return isValid;
 })
 .sort((a: any, b: any) => {
 ...
 console.warn(` [WARN] Error obteniendo progreso para curso ${courseId}: ${progressResponse.status}`);
 const errorData = await progressResponse.json();
 console.warn(` Error data:`, errorData);
 }
 } else {
 console.warn(` [WARN] No se pudo obtener userId para curso ${courseId}`);
 }
 }
 } catch (e) {
 ...
 if (!lesson || !lesson.lessonId) {
 console.warn(` [WARN] Lección sin ID válido - omitida`);
 skippedCount++;
 return;
 }

 if (!lesson.lessonTitle || lesson.lessonTitle.trim() === '') {
 console.warn(` [WARN] Lección ${lesson.lessonId} sin título válido - omitida`);
 skippedCount++;
 return;
 }
 ...
 console.warn(` [WARN] Lección duplicada detectada y omitida: ${lesson.lessonId} - ${lesson.lessonTitle} (ya agregada anteriormente)`);
 duplicateCount++;
 return;
 }
 ...
 console.error(`[ERROR] Error de orden: Módulo ${prev.moduleOrderIndex} después de ${curr.moduleOrderIndex} en curso ${prev.courseId}`);
 orderIsCorrect = false;
 } else if (prev.moduleOrderIndex === curr.moduleOrderIndex && prev.lessonOrderIndex >= curr.lessonOrderIndex) {
 console.error(`[ERROR] Error de orden: Lección ${prev.lessonOrderIndex} después de ${curr.lessonOrderIndex} en módulo ${prev.moduleOrderIndex}`);
 orderIsCorrect = false;
 }
 }
 }
 ...
 if (!isValid) {
 console.warn(`[WARN] Lección inválida filtrada:`, {
 lessonId: l?.lessonId,
 lessonTitle: l?.lessonTitle,
 lessonOrderIndex: l?.lessonOrderIndex
 });
 ...
 console.warn(`[WARN] ${invalidLessonsCount} lecciones inválidas filtradas antes de la distribución`);
 }
 ...
 console.warn(`[WARN] [Distribución] Quedaron ${unassignedCount} lecciones sin asignar por falta de espacio en el calendario.`);
 } else {
 }
 ...
 console.warn(`[WARN] Lección duplicada detectada y omitida: ${lesson.lessonId} - ${lesson.lessonTitle}`);
 currentLessonIndex++;
 ...
 console.warn(`[WARN] Slot ${slotIndex} no tiene lecciones asignadas`);
 }
 });
 ...
 console.warn(`[WARN] Después de la redistribución, aún quedan ${stillRemaining} lecciones sin asignar. Usando TODOS los slots disponibles...`);
 ...
 console.warn(`[WARN] Aún quedan ${stillRemainingAfter} lecciones. Intentando llenar slots existentes al máximo...`);
 ...
 if (finalRemaining > 0) {
 console.error(`[ERROR] CRITICO: Aún quedan ${finalRemaining} lecciones sin asignar después de usar TODOS los slots disponibles`);
 */
 ...
 lesson.lessonOrderIndex >= 0;
 if (!isValid) {
 console.warn(`[WARN] Lección inválida filtrada de distribución:`, lesson);
 }
 return isValid;
 }).map(lesson => {
 // ✅ CORRECCIÓN: Asegurar que lessonOrderIndex sea válido
 ...
 calendarMessage += `\n\n[WARN]  **ALERTA IMPORTANTE:** Quedan ${remainingLessons} lecciones pendientes por asignar de ${validPendingLessons.length} totales. `;
...
¿En qué horario te funciona mejor: mañana, tarde o noche?`;
 ...
 console.error('[ERROR] [disconnectCalendar] Error desconectando calendario:', error);
 ...
 const SofLIAResponse = `Entendido, no hay problema.${profileInfo}
...
(Por ejemplo: "Lunes, miércoles y viernes por la noche" o "Fines de semana por la mañana")`;
 ...
 const SofLIAResponse = 'Entendido. Cuéntame: ¿Qué días de la semana prefieres estudiar y en qué horarios? (Por ejemplo: "Lunes a viernes por la noche")';
 setConversationHistory(prev => [...prev, { role: 'assistant', content: SofLIAResponse }]);
 ...
 const parseSofLIAScheduleResponse = (SofLIAResponse: string): StoredLessonDistribution[] | null => {
 try {
 ...
 const hasSchedulePatterns = /(?:lunes|martes|miércoles|miercoles|jueves|viernes|sábado|sabado|domingo)\s+\d{1,2}/i.test(SofLIAResponse) ||
 /\d{1,2}\s+(?:de\s+)?(?:enero|febrero|marzo|abril|mayo|junio|julio|agosto|septiembre|octubre|noviembre|diciembre)/i.test(SofLIAResponse) ||
 /\d{1,2}\/\d{1,2}\/\d{4}/.test(SofLIAResponse) ||
 /(?:de\s+)?\d{1,2}:\d{2}\s+(?:a\.?m\.?|p\.?m\.?|a\s+las?\s+\d{1,2})/i.test(SofLIAResponse) ||
 /\d{1,2}(?::\d{2})?\s*(?:a\.?m\.?|p\.?m\.?)\s+(?:a|hasta)\s+\d{1,2}(?::\d{2})?\s*(?:a\.?m\.?|p\.?m\.?)/i.test(SofLIAResponse) ||
 /horario/i.test(SofLIAResponse) && /\d{1,2}/.test(SofLIAResponse);
 ...
 let match;
 while ((match = scheduleBlockPattern.exec(SofLIAResponse)) !== null) {
 ...
 const nextLines = SofLIAResponse.substring(matchEnd, matchEnd + 500).split('\n').slice(0, 10);
 ...
 const lines = SofLIAResponse.split('\n');
 ...
 if (hasSchedulePatterns) {
 console.warn('[WARN] Se detectaron patrones de horarios pero no se extrajeron horarios válidos');
 console.warn(' Respuesta de SofLIA (primeros 500 caracteres):', SofLIAResponse.substring(0, 500));
 }

 return null;
 } catch (error) {
 console.error('[ERROR] Error parseando respuesta de SofLIA:', error);
 return null;
 }
 };
 ...
 console.warn(`[WARN] [parseDateStr] No se pudo parsear fecha: "${dateStr}"`);
 return null;
 };
 ...
 console.warn(`[WARN] Horario inválido: ${timeStr}`);
 ...
 if (!timeMatch) {
 console.warn(`[WARN] No se pudo extraer hora:minuto de: ${timeStr}`);
 return { hours: 9, minutes: 0 };
 }
 ...
 console.warn(`[WARN] Valores de hora inválidos: ${hours}:${minutes} de: ${timeStr}`);
 return { hours: 9, minutes: 0 };
 }
 ...
 if (hours < 0 || hours > 23) {
 console.warn(`[WARN] Hora fuera de rango después de conversión: ${hours}:${minutes} de: ${timeStr}`);
 return { hours: 9, minutes: 0 };
 }
 ...
 if (validLessons.length === 0) {
 console.warn(`[WARN] Slot sin lecciones válidas: ${slot.dateStr} ${slot.startTime}`);
 ...
 console.error('[ERROR] Error del servidor (texto no JSON):', errorText);
 ...
 console.error('[ERROR] Error procesando respuesta del servidor:', e);
 ...
 console.warn(`[WARN] [Insert Events] Fecha inválida: "${item.dateStr}", usando fecha actual`);
 baseDate = new Date();
 }
 ...
 console.error(`[ERROR] [Insert Events] Error: ${insertResult.error || 'Error desconocido'}`);
 ...
 setConversationHistory(prev => [...prev, {
 role: 'assistant',
 content: '[WARN]  Tu conexión con el calendario ha expirado. El plan se guardó pero no se pudieron crear los eventos. Reconecta tu calendario para sincronizar.'
 }]);
 }
 }
 } catch (insertError) {
 console.error('[ERROR] [Insert Events] Error insertando eventos:', insertError);
 ...
 console.error('[ERROR] Error al redirigir:', redirectError);
 ...
 // Función para enviar mensajes a SofLIA
 ...
 // ✅ NUEVO: Detectar si el usuario está aceptando ampSofLIAr horarios después de mensaje de deadline excedido
 // Esto ocurre cuando:
 // 1. El usuario dice "sí", "ok", "acepto", etc.
 // 2. NO hay savedLessonDistribution (porque el deadline se excedió y no se generó plan)
 // 3. El último mensaje de SofLIA mencionaba ampSofLIAr horarios o fecha límite
 ...
 // Verificar si el último mensaje de SofLIA era sobre deadline excedido
 ...
 lastAssistantMsg.content.includes('ampSofLIAr tus horarios') ||
 ...
 const proposalMessage = `${message}\n\n[SISTEMA: El usuario ACEPTÓ ampSofLIAr sus horarios. ` +
 ...
 `NO vuelvas a preguntar si quiere ampSofLIAr - YA DIJO QUE SÃ. ` +
 ...
 // Buscar en el último mensaje de SofLIA los datos JSON de alternativas
 const lastAssistantMsg = conversationHistory.filter(m => m.role === 'assistant').pop();
 ...
 // Enriquecer el mensaje para que SofLIA sepa que debe regenerar el plan con esa opción
 ...
 console.warn(`[WARN] No se pudo obtener el plan activo:`, error);
 }
 }
 ...
 console.warn(`[WARN] Errores al actualizar:`, updateData.data.errors);
 ...
 console.warn(`[WARN] La actualización falló:`, updateData);
 ...
 console.error(`[ERROR] Error actualizando sesiones en BD (${updateResponse.status}):`, errorText);
 ...
 console.error('[ERROR] Error actualizando sesiones en BD:', updateError);
 ...
 // Esto es importante si SofLIA acaba de actualizar el estado
 ...
 setConversationHistory(prev => [...prev, {
 role: 'assistant',
 content: 'Lo siento, no hay horarios para guardar. Por favor, pide a SofLIA que genere un plan de estudios primero.'
 }]);
 setIsProcessing(false);
 return;
 }
 ...
 // Formato OPTIMIZADO para SofLIA - solo primeros y últimos slots para evitar error 400
 ...
 if (isB2BSummary) {
 distributionSummary += `[WARN]  **ALERTA CRÃTICA:** Se han asignado ${totalLessonsAssigned} de ${savedTotalLessons} lecciones. Faltan ${savedTotalLessons - totalLessonsAssigned} por asignar.\n`;
 ...
 distributionSummary += `[WARN]  Se han asignado ${totalLessonsAssigned} de ${savedTotalLessons} lecciones. Faltan ${savedTotalLessons - totalLessonsAssigned} por asignar.\n`;
 }
 }
 ...
 // Marcar que vamos a mostrar el resumen final después de que SofLIA responda
 // Esto se marcará como true cuando SofLIA responda con el resumen
 ...
 if (savedCalendarData && Object.keys(savedCalendarData).length > 0) {
 addScheduleContext += `**[WARN]  VALIDACIÓN DE CONFLICTOS:**\n`;
 ...
 // ✅ NUEVO: Detección de bucles - si SofLIA está repitiendo preguntas similares
 const lastAssistantMessages = conversationHistory
 .filter(m => m.role === 'assistant')
 .slice(-5);

 // Patrones que indican bucle (SofLIA repitiendo la misma pregunta)
 const loopPatterns = [
 /confirmes los días/i,
 /te refieres a todos los/i,
 /qué días.*prefieres/i,
 /qué horario.*funciona/i,
 /podrías.*ampSofLIAr.*horarios/i,
 /necesito que me confirmes/i,
 ];

 // Contar cuántos mensajes recientes de SofLIA tienen patrones de bucle
 ...
 // Construir contexto de lecciones pendientes para el prompt
 // Usa el hook SofLIAData como fuente primaria (datos exactos de BD)
 const sendMsgLessonsContext = SofLIAData.isReady && SofLIAData.lessons.length > 0
 ? SofLIAData.getLessonsForPrompt()
 : pendingLessonsRef.current.length > 0
 ? pendingLessonsRef.current.map(l => `- ${l.lessonTitle} (${l.durationMinutes || 15} min) - Módulo: ${l.moduleTitle}`).join('\n')
 : 'No hay lecciones pendientes definidas aún.';

 // ✅ REFUERZO DE CONTEXTO: Si ya hay un plan parseado en memoria, recordárselo a SofLIA
 // Esto evita que SofLIA "olvide" el plan cuando el usuario confirma ("sí") y el preCalculatedPlanContext desaparece
 let existingPlanContext = '';
 if (savedLessonDistribution.length > 0) {
 existingPlanContext = `\n\nâ•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•\nðŸ“ PLAN PROPUESTO ACTUALMENTE (EN MEMORIA)\nâ•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•\nSofLIA, ya has propuesto este plan y el usuario lo tiene en pantalla:\n`;
 ...
 existingPlanContext += `[WARN]  INSTRUCCIÓN CRÃTICA DE CONFIRMACIÓN: Si el usuario dice "sí", "ok", "me gusta" o confirma:\n1. NO recalculas nada.\n2. NO digas que hubo un error.\n3. Confirma que guardas este plan.\n4. Muestra entusiasmo.\n`;
 }
 ...
 dueDateContext = `\n\n🚨 FECHA LÃMITE OBLIGATORIA: ${dueDateFormatted}\n[WARN]  NUNCA programar lecciones después de esta fecha.\n[WARN]  La fecha de finalización del plan DEBE ser ANTERIOR a ${dueDateFormatted}.`;
 }

 // ðŸ•µ Detección automática de preferencias para Generador Determinista
 ...
 if (daysMatch && daysMatch.length > 0 && SofLIAData.lessons.length > 0) {
 try {
 ...
 alternativeOptions = `[WARN]  ADVERTENCIA: La fecha límite es muy ajustada.\n`;
 ...
 `[WARN]  INSTRUCCIÓN CRÃTICA PARA SofLIA:\n` +
 ...
 if (data.conversationId && !SofLIAConversationId) {
 setSofLIAConversationId(data.conversationId);
 }
 ...
 // ✅ SANITIZE: Limpiar respuesta de SofLIA para eliminar menciones de festivos
 ...
 SofLIAResponse = SofLIAResponse.replace(/^\*?\s*📅?\s*\*?\*?Jueves\s+1\s*:?.*$/gim, ''); // Jueves 1
 SofLIAResponse = SofLIAResponse.replace(/^\*?\s*\*?\*?Jueves\s+1\s*:?.*$/gim, '');
 SofLIAResponse = SofLIAResponse.replace(/\*\s*08:00.*Jueves\s+1.*\n/gi, '');
 SofLIAResponse = SofLIAResponse.replace(/1\s+de\s+enero\s*[-â€“â€”]\s*\d+\s+de\s+enero/gi, (match) => {
 // Reemplazar "1 de enero - 6 de enero" por "2 de enero - 6 de enero"
 return match.replace(/1\s+de\s+enero/i, '2 de enero');
 });
 SofLIAResponse = SofLIAResponse.replace(/Fechas:\s*1\s+de\s+enero/gi, 'Fechas: 2 de enero');
 // Limpiar líneas vacías extras causadas por las eliminaciones
 SofLIAResponse = SofLIAResponse.replace(/\n{3,}/g, '\n\n');

 setConversationHistory(prev => [...prev, { role: 'assistant', content: SofLIAResponse }]);

 // ✅ NUEVO: Parsear respuesta de SofLIA para extraer horarios y actualizar savedLessonDistribution
 // Ejecutar siempre que haya horarios en la respuesta o cuando sea relevante (agregar/confirmar horarios)

 const extractedSchedulesRaw = parseSofLIAResponseToSchedules(SofLIAResponse);
 ...
 // Detectar si SofLIA está mostrando un resumen completo (todos los horarios)
 // Indicadores: menciona "RESUMEN", "DISTRIBUCIÓN", "todos los horarios", o tiene muchos horarios
 const isCompleteSummary = SofLIAResponse.includes('RESUMEN') ||
 SofLIAResponse.includes('resumen') ||
 SofLIAResponse.includes('DISTRIBUCIÓN') ||
 SofLIAResponse.includes('distribución') ||
 SofLIAResponse.includes('todos los horarios') ||
 SofLIAResponse.includes('horarios:') ||
 SofLIAResponse.includes('sesiones programadas') ||
 SofLIAResponse.includes('plan de estudios') ||
 SofLIAResponse.includes('sesiones generadas') ||
 (extractedSchedules.length >= 5 && savedLessonDistribution.length > 0);


 // Si es un resumen completo o si estamos agregando horarios (SofLIA muestra todos), reemplazar completamente
 ...
 // Preservar lecciones de horarios existentes si SofLIA no las mencionó explícitamente
 ...
 // SofLIA generalmente solo menciona "Lección X" sin el título completo, por lo que
 ...
 // ✅ CRÃTICO: SIEMPRE preservar las lecciones existentes con sus nombres completos
 // SofLIA generalmente no menciona los títulos completos, solo "Lección X" o títulos cortados
 ...
 // ✅ CRÃTICO: Si SofLIA mostró un resumen pero no extrajimos horarios, 
 // y el usuario está confirmando, debemos preservar el estado actual
 // Esto puede pasar si SofLIA no usa el formato exacto que el parser espera
 if (isConfirmingSchedules || hasShownFinalSummary) {
 }
 }

 // Si fue una confirmación de horarios y SofLIA está mostrando el resumen final, marcar que se mostró
 if (isConfirmingSchedules && !hasShownFinalSummary && (
 SofLIAResponse.includes('RESUMEN') ||
 SofLIAResponse.includes('resumen') ||
 SofLIAResponse.includes('distribución') ||
 ...
 // Si fue una solicitud de agregar horarios, la respuesta de SofLIA ya incluye todos los horarios (existentes + nuevos)
 ...
 // Si el usuario dijo "sí/ok" y SofLIA confirma guardado, ejecutar la acción real
 ...
 const SofLIAConfirmsSaving = SofLIAResponse.toLowerCase().match(/(guardad|guardar|xito|comenzar|dashboard|redireccion|creado|alegra|disfrut)/i);

 if (isUserConfirmation && SofLIAConfirmsSaving && savedLessonDistribution.length > 0) {
 // Ejecutar guardado (pequeño delay para que SofLIA termine de hablar/mostrar mensaje)
 setTimeout(() => {
 ...
 console.warn('[WARN] No hay lecciones para guardar (cleanDistribution empty)');
 return;
 }
 ...
 console.warn(`[WARN] Slot ${idx} ignorado por falta de fecha/hora:`, slot);
 return null;
 }
 ...
 console.error(`[ERROR] Formato inválido en slot ${idx}: Fecha=${cleanDate}, Hora=${cleanStart}-${cleanEnd}`);
 return null;
 }
 ...
 console.error(`[ERROR] Datos no numéricos en slot ${idx} tras reparación`);
 return null;
 }
 ...
 console.error(`[ERROR] Fecha inválida (Date obj) en slot ${idx}`);
 return null;
 }
 ...
 console.error(`[ERROR] Error inesperado procesando sesión ${idx}:`, e);
 return null;
 }
 }).filter(Boolean);

 if (sessions.length === 0) {
 console.error("[ERROR] FATAL: Todas las sesiones fueron descartadas. Revise los logs anteriores.");
 ...
 description: `Plan generado por SofLIA el ${new Date().toLocaleString('es-ES')}`,
 ...
 console.error('[ERROR] Error API:', responseData.error);
 ...
 console.error('[ERROR] Error crítico en guardado:', error);
 ...
 >
 {/* Esfera animada con avatar de SofLIA */}
 ...
 {/* Interfaz de conversación con SofLIA */}
 {
 showConversation && (
 ...
 {/* Header */}
 <div id="SofLIA-planner-header" className="flex-shrink-0 z-10 bg-white dark:bg-[#0F1419] backdrop-blur-xl border-b border-[#E9ECEF] dark:border-[#6C757D]/30 px-3 py-3 sm:px-4 sm:py-4">
 ...
 {formatSofLIAMessage(msg.content)}
 </div>
 ) : (
 ...
 {/* ✅ NUEVO: Botones inline de selección de ritmo de estudio (con avatar de SofLIA) */}
 ...
 className="relative w-8 h-8 sm:w-10 sm:h-10 rounded-full overflow-hidden border-2 border-[#0A2540]/30 dark:border-[#00D4B3]/40 flex-shrink-0 shadow-lg shadow-[#0A2540]/20 dark:shadow-[#00D4B3]/20 hidden sm:block"
 >
 <Image src="/SofLIA-avatar.png" alt="SofLIA" fill sizes="40px" className="object-cover" />
 </motion.div>

 {/* Avatar de SofLIA - Mobile */}
 <div className="relative w-6 h-6 rounded-full overflow-hidden border border-[#0A2540]/30 dark:border-[#00D4B3]/40 flex-shrink-0 sm:hidden self-start mt-1">
 <Image src="/SofLIA-avatar.png" alt="SofLIA" fill sizes="24px" className="object-cover" />
 </div>

 {/* Contenedor de botones estilo mensaje de SofLIA */}
 <motion.div
 initial={{ scale: 0.9, opacity: 0 }}
 animate={{ scale: 1, opacity: 1 }}
