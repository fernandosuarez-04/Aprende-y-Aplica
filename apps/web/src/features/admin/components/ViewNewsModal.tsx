    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-gray-800 rounded-lg shadow-xl max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-gray-700">
          <h2 className="text-xl font-semibold text-white">Detalles de la Noticia</h2>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="flex items-start justify-between">
            <div className="flex-1">
              ...
              <div className="flex items-center gap-3 mb-4">
                ...
                <span className="text-gray-400 text-sm">{news.language}</span>
          ...
          {news.intro && (
            <div>
              <h3 className="text-lg font-semibold text-white mb-2">Introducción</h3>
              <p className="text-gray-300 leading-relaxed">{news.intro}</p>
            </div>
          )}
          ...
          {news.subtitle && (
            <div>
              <h3 className="text-lg font-semibold text-white mb-2">Subtítulo</h3>
              <p className="text-gray-300 leading-relaxed">{news.subtitle}</p>
            </div>
          )}
          ...
          {news.sections && (
            <div>
              <h3 className="text-lg font-semibold text-white mb-2">Secciones</h3>
          ...
          {news.tldr && (
            <div>
              <h3 className="text-lg font-semibold text-white mb-2">TLDR</h3>
              <div className="bg-gray-700 rounded-lg p-4">
                <pre className="text-gray-300 text-sm overflow-x-auto">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="bg-gray-700 rounded-lg p-4 text-center">
                <div className="text-2xl font-bold text-white">{news.view_count?.toLocaleString() || 0}</div>
                <div className="text-sm text-gray-400">Vistas</div>
              </div>
              
              <div className="bg-gray-700 rounded-lg p-4 text-center">
                <div className="text-2xl font-bold text-white">{news.comment_count?.toLocaleString() || 0}</div>
                <div className="text-sm text-gray-400">Comentarios</div>
              </div>
              
              <div className="bg-gray-700 rounded-lg p-4 text-center">
                <div className="text-2xl font-bold text-white">{news.language.toUpperCase()}</div>
              ...
              <div className="bg-gray-700 rounded-lg p-4 text-center">
                ...
                <div className="text-sm text-gray-400">Fecha de publicación</div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-gray-700 rounded-lg p-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              <div>
                ...
                <span className="text-white ml-2 font-mono">{news.id}</span>
              </div>
              <div>
                <span className="text-gray-400">Slug:</span>
                <span className="text-white ml-2 font-mono">{news.slug}</span>
              </div>
              <div>
                <span className="text-gray-400">Creada:</span>
                <span className="text-white ml-2">{new Date(news.created_at).toLocaleString()}</span>
              </div>
              <div>
                <span className="text-gray-400">Actualizada:</span>
                <span className="text-white ml-2">{new Date(news.updated_at).toLocaleString()}</span>
              </div>
              <div>
                <span className="text-gray-400">Creada por:</span>
                <span className="text-white ml-2 font-mono">{news.created_by}</span>
              </div>
            </div>
          </div>
        </div>
