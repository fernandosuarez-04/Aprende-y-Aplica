...
import { AdminReelsService, AdminReel, AdminReelStats, CreateReelData, UpdateReelData } from '../services/adminReels.service'

export function useAdminReels() {
  const [reels, setReels] = useState<AdminReel[]>([])
  const [stats, setStats] = useState<AdminReelStats | null>(null)
  ...
  const fetchReels = async () => {
    try {
      ...
      const [reelsData, statsData] = await Promise.all([
        AdminReelsService.getReels(),
        AdminReelsService.getStats()
      ])
      setReels(reelsData)
      setStats(statsData)
    } catch (err) {
  ...
  const createReel = async (data: CreateReelData) => {
    try {
      const newReel = await AdminReelsService.createReel(data)
  ...
  const updateReel = async (id: string, data: UpdateReelData) => {
    try {
      const updatedReel = await AdminReelsService.updateReel(id, data)
  ...
  const deleteReel = async (id: string) => {
    try {
      await AdminReelsService.deleteReel(id)
  ...
  const toggleReelStatus = async (id: string) => {
    try {
      const updatedReel = await AdminReelsService.toggleReelStatus(id)
      setReels(prev => prev.map(reel => 
        reel.id === id ? updatedReel : reel
      ))
  ...
  const toggleReelFeatured = async (id: string) => {
    try {
      const updatedReel = await AdminReelsService.toggleReelFeatured(id)
      setReels(prev => prev.map(reel => 
        reel.id === id ? updatedReel : reel
      ))
